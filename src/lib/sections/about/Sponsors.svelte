<script lang="ts">
	...
	import hotmilkUrl from "$lib/assets/hotmilk.svg";
	...
	function copyToClipboard() {
		try {
			navigator.clipboard.writeText("hello@flux.sh");
			copied = true;
			ToastManager.add({
				type: "success",
...
<Panel class="flex flex-col gap-3 p-6 min-h-[280px]">
	<h2 class="text-2xl font-bold flex items-center">
		<div
			class="rounded-full glass-icon-pink p-2 inline-block mr-3 w-10 h-10"
		>
			<PiggyBankIcon />
		</div>
	...
	<div class="mt-2 [&>*]:font-normal h-full flex justify-between flex-col">
		<div class="flex gap-3 justify-center text-lg">
			...
			>
				<img src={hotmilkUrl} alt="Hot Milk" class="w-full h-16" />
			</a>
		</div>
		<p class="text-muted">
			{@html sanitize(link(
