...
const __PUB_FLUXD_URL__ = "";

const LOCATIONS = [
	{
		latitude: 49.0976,
		longitude: 12.4869,
		url: "https://eu.vertd.flux.sh",
	},
	{
		latitude: 47.6587,
		longitude: -117.426,
		url: "https://usa.vertd.flux.sh",
	},
];
...
export class VertdInstance {
	...
	public load() {
		...
		if (!ls) {
			const isCustomUrl =
				__PUB_FLUXD_URL__ && __PUB_FLUXD_URL__ !== "https://vertd.flux.sh";
			if (isCustomUrl) {
	...
	public async url() {
		...
		switch (this.inner.type) {
			...
			case "eu": {
				return "https://eu.vertd.flux.sh";
			}

			case "us": {
				return "https://usa.vertd.flux.sh";
			}
