...
import { videoFormats } from "$lib/converters/ffmpeg.svelte";
...
class Files {
	...
	private _addThumbnail = async (file: VertFile) => {
		this.thumbnailQueue.add(async () => {
			const isVideo = videoFormats.includes(file.from.replace(".", "").toLowerCase());
			const isAudio = !isVideo && converters
				.find((c) => c.name === "ffmpeg")
				?.supportedFormats.filter((f) => f.isNative)
				.map((f) => f.name)
				?.includes(file.from.replace(".", "").toLowerCase());
	...
	public async downloadAll() {
		...
		const filenameFormat = settings.filenameFormat || "FLUX_%name%";
